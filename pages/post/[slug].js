...
export default function Post(props) {
    ...
    return (
    <>
        <div className=" sm:flex-row mt-8">
            <div className="sm:w-full bg-white/100 shadow">
                <h1 className="text-xl sm:text-3xl font-black font-roboto px-8 pt-8 mt-8 "> {props.post.title}</h1>
                <p className="px-8">
                ...
                <div className="text-black px-8 pb-8 font-serif font-medium text-lg space-y-4 sm:max-w-[720px] mx-auto" dangerouslySetInnerHTML={{ __html: props.post.html }}>
                </div>
            </div>
            <div className="flex flex-col sm:w-full bg-black/50">
            <div className="bg-black/75 shadow">
                <h3 className="font-semibold m-4 text-2xl text-white"> Related Posts</h3>
            </div>
            {props.posts.slice(0,3).map(post => (
                <div className="hover:brightness-75 sm:w-1/3">
                <Link href={"/post/" + post.slug}>
                    <img src={post.feature_image} className="w-full"/>
                    <div className="px-4 bg-black/50 text-white p-4 w-full flex flex-col">
